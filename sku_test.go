...
func Test_SKU_GetCapabilityDate(t *testing.T) {
	tests := map[string]struct {
		...
		"nil capability value": {
			capabilities: &[]compute.ResourceSkuCapabilities{
				{
					Name: to.StringPtr("RetirementDateUtc"),
				},
			},
			capability: "RetirementDateUtc",
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			sku := SKU(compute.ResourceSku{Capabilities: test.capabilities})
	...
	cases := map[string]struct {
		...
		"API capability list": {
			sku: compute.ResourceSku{
				Size: to.StringPtr("D32pvds_v7"),
				Capabilities: &[]compute.ResourceSkuCapabilities{
					{Name: to.StringPtr(SupportedVirtualizationTypes), Value: to.StringPtr(NestedVirtualization + "," + DirectVirtualization)},
				},
			},
			expect: true,
		},
	...
	cases := map[string]struct {
		...
		"API capability": {
			sku: compute.ResourceSku{
				...
				Capabilities: &[]compute.ResourceSkuCapabilities{
					{Name: to.StringPtr(CapabilityConfidentialComputingType), Value: to.StringPtr(ConfidentialComputingTypeSNP)},
				},
			},
			expect: true,
		},
		...
		"unsupported confidential type": {
			sku: compute.ResourceSku{
				Size: to.StringPtr("D2ds_v7"),
				Capabilities: &[]compute.ResourceSkuCapabilities{
					{Name: to.StringPtr(CapabilityConfidentialComputingType), Value: to.StringPtr("TDX")},
				},
			},
		},
	}
...
func Test_SKU_IsNestedSNPSupported(t *testing.T) {
	cases := map[string]struct {
		...
		"DCas v6 4-vCPU confidential child":  {size: "DC4as_cc_v6", expect: true},
		"DCas v6 32-vCPU confidential child": {size: "DC32as_cc_v6", expect: true},
